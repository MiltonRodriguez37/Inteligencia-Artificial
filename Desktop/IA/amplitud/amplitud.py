...
def bfs(estadoIncial):
    global MaxFrontera, NodoObjetivo, MaxBusquedaProf

    caminoVisitado = set()
    Cola = deque([EstadoPuzzle(estadoIncial,None,None,0,0,0)])
    ...
    while Cola:
        ...
        if nodo.estado == EstadoObjetivo:
            NodoObjetivo = nodo
            return Cola
        posibleCaminos = subNodos(nodo)
        for camino in posibleCaminos:
            if camino.mapa not in caminoVisitado:
                Cola.append(camino)
                caminoVisitado.add(camino.mapa)
                if camino.profundidad > MaxBusquedaProf:
                    MaxBusquedaProf = MaxBusquedaProf + 1
        if len(Cola) > MaxFrontera:
            MaxFrontera = len(Cola)
...
def main():
    ...
    print("\nResultados Búsqueda en Amplitud: \n")
